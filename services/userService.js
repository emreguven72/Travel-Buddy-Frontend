...
//Desktop ip address: 192.168.1.248

export default class UserService {
    async getAllUsers() {
        try {
            return await axios.get('http://192.168.1.248:8080/api/users/getAll');
        } catch(e) {
    ...
    async createUser(email,password,username,name,surname,phoneNumber) {
        try {
            ...
            await axios.post('http://192.168.1.248:8080/api/users/add', {
                email: email,
                password: password,
                username: username,
                name: name,
    ...
    async deleteUser(id) {
        try {
            return await axios.delete(`http://192.168.1.248:8080/api/users/delete?id=${id}`)
    ...
    async getById(id) {
        try{
            const response = await axios.get(`http://192.168.1.248:8080/api/users/getById?id=${id}`);
