...
const useTravelStore = create((set) => ({
    travels: null,
    fetch: async() => {
        const response = await axios.get('http://192.168.0.12:8080/api/travels/getAll');
        set({ travels: await response.data });
    },
    fetchByLocations: async(startLocation,endLocation) => {
        const response = await axios.get(`http://192.168.0.12:8080/api/travels/getByLocations?startLocation=${startLocation}&endLocation=${endLocation}`)
        set({ travels: await response.data })
    },
